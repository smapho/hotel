...
const ROOM_TYPES = [
  { roomClass: "single", roomName: "シングル", planName: "室料のみプラン", multiplier: 0.65, withBreakfast: false, withDinner: false, payment: "1" },
  { roomClass: "double", roomName: "ダブル", planName: "室料のみプラン", multiplier: 0.9, withBreakfast: false, withDinner: false, payment: "1" },
  { roomClass: "twin", roomName: "ツイン", planName: "朝食付きプラン", multiplier: 1.0, withBreakfast: true, withDinner: false, payment: "1" },
  { roomClass: "japanese", roomName: "和室", planName: "夕朝食付きプラン", multiplier: 1.2, withBreakfast: true, withDinner: true, payment: "0" },
  { roomClass: "deluxe-twin", roomName: "デラックスツイン", planName: "朝食付きプラン", multiplier: 1.5, withBreakfast: true, withDinner: false, payment: "2" },
  { roomClass: "suite", roomName: "スイート", planName: "室料のみプラン", multiplier: 2.3, withBreakfast: false, withDinner: false, payment: "2" },
];
...
function buildHotelNo({ regionCode, cityCode, detailCode, hotelIndex }) {
  return `mock-${regionCode}-${cityCode ?? ""}-${detailCode ?? ""}-${hotelIndex}`;
}

function parseHotelNo(hotelNo) {
  const parts = hotelNo.split("-");
  const [, regionCode, cityCode, detailCode, hotelIndex] = parts;
  return {
    regionCode,
...
function generateRoomsForHotel({ regionCode, hotelIndex, nights, adultNum, roomNum, roomCount }) {
  return ROOM_TYPES.slice(0, roomCount).map((roomType) => {
    ...
    return {
      ...
      reserveUrl: null,
      withBreakfast: roomType.withBreakfast,
      withDinner: roomType.withDinner,
      payment: roomType.payment,
      pointRate: 1,
      planContents: null,
      adultNum,
      roomNum,
...
// 実際の楽天API(searchPattern=0)はホテル一覧では1ホテルにつき1室しか返らないため、
// モックでも一覧は概要(代表1室)のみとし、全部屋タイプはgetHotelRoomsで取得する形に合わせる。
export async function searchHotels({ regionCode, cityCode, detailCode, checkinDate, checkoutDate, adultNum = 1, roomNum = 1 }) {
  ...
  const hotels = HOTEL_NAME_TEMPLATES.map((nameFn, hotelIndex) => {
    ...
    const summaryRoom = generateRoomsForHotel({ regionCode, hotelIndex, nights, adultNum, roomNum, roomCount: 1 });

    return {
      hotelNo: buildHotelNo({ regionCode, cityCode, detailCode, hotelIndex }),
...
// ホテル1件分の全部屋タイプを取得する(HotelCardのクリック展開用)。
export async function getHotelRooms({ hotelNo, checkinDate, checkoutDate, adultNum = 1, roomNum = 1 }) {
  const { regionCode, hotelIndex } = parseHotelNo(hotelNo);
  const nights = getNightsInRange(checkinDate, checkoutDate);
  const hotelSeed = hotelNo;
  const roomCount = 3 + Math.floor(seededRandom(hotelSeed) * 2);
  const rooms = generateRoomsForHotel({ regionCode, hotelIndex, nights, adultNum, roomNum, roomCount });

  return { provider: "mock", hotelNo, checkinDate, checkoutDate, nights, rooms };
}
