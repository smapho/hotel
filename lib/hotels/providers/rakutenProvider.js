...
// VercelはデプロイごとにVERCEL_URLを自動設定するため、ローカル開発時のフォールバックのみ用意する。
...
async function fetchNightFromApi({ regionCode, night, adultNum, roomNum, applicationId, affiliateId, accessKey }) {
...
export async function searchHotels({ regionCode, checkinDate, checkoutDate, adultNum = 1, roomNum = 1 }) {
  ...
  for (const night of nights) {
    ...
    if (!nightlyHotels) {
      ...
      nightlyHotels = await fetchNightFromApi({ regionCode, night, adultNum, roomNum, applicationId, affiliateId, accessKey });
      calledApi = true;
