...
async function fetchNightFromApi({ regionCode, cityCode, night, adultNum, roomNum, applicationId, affiliateId, accessKey }) {
...
export async function searchHotels({ regionCode, cityCode, checkinDate, checkoutDate, adultNum = 1, roomNum = 1 }) {
  ...
  // regionCodeだけでなくcityCodeも含めてキャッシュキーにする(市区町村ごとに結果が変わるため)。
  const areaKey = cityCode ? `${regionCode}:${cityCode}` : regionCode;
  ...
  for (const night of nights) {
    ...
    if (!nightlyHotels) {
      ...
      nightlyHotels = await fetchNightFromApi({ regionCode, cityCode, night, adultNum, roomNum, applicationId, affiliateId, accessKey });
      calledApi = true;
